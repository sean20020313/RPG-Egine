...
**關卡制：** 每關地圖上約 **4～6 隻**一般魔物；全部擊敗後會出現**首領**（獨立造型、約 2 倍體型、光環與金色血條）。擊敗首領後進入下一關。
...
**地圖道具：** 每關會散落 3～5 個寶箱（藥草、治療／魔力藥水等），**走到同一格**即可拾取。

**魔物種類：** 史萊姆、哥布林、野狼、石像鬼、蝙蝠、骷髏兵、毒蜘蛛、獸人等；受擊時魔物會**撲向玩家**並顯示紅色斬擊特效。
...
**目前 `play/` 狀態**：**固定 2D 地圖**、關卡制（每關 4～6 隻小怪 + 首領）、**鍵盤**移動與攻擊（見上方「鍵盤操作」）；全中文 HUD：**主選單**、**重新開始**、**任務 / 背包**。
